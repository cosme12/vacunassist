...
def login_required(func):
    ...
    def decorated_function(*args, **kwargs):
        if 'dni' not in session:
            ...
            if request.path != url_for('perfil') and request.path != url_for('logout') and 'eliminar_cuenta' not in request.path:
                usuario = get_user_data(session['dni'])
                edad = edad_de_usuario(usuario['id'])[0]
                ...
                if not vucuna_covid_1 and not tiene_turno_covid and (usuario['paciente_de_riesgo'] or edad > 60):
                    flash("Para poder acceder al resto del sitio debe reservar un turno para aplicarse la primera dosis de la vacuna de COVID-19.", "danger")
