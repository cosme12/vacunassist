...
def test_registrar_paciente_sin_historial_identidad_incorrecta(client, create_db):
    response = client.post('/registro', data={
        'dni': '00000000',
        'nombre': 'Juan',
        'apellido': 'Perez',
    ...
    assert response.status_code == 200
    #debuggear_respuesta(response.get_data(as_text=True))
    assert response.request.path == '/registro' # redirected to login
    html = response.get_data(as_text=True)
    assert f'No pudo validarse su identidad' in html
...
def test_registrar_paciente_con_historial_vacunas_repetidas(client, create_db):
    ...
    assert response.status_code == 200
    html = response.get_data(as_text=True)
    assert f'No puede seleccionar más de una vez la misma vacuna.' in html
